...
func (factory *Factory) Dial(dialCtx context.Context) (*Session, error) {
	...
	// dialer is the effective websocket dialer for this attempt.
	dialer := factory.Dialer
	if dialer == nil {
		dialer = websocket.DefaultDialer
	}

	// conn is the raw connected websocket returned by gorilla.
...
func (factory *Factory) applyConnectionTuning(conn *websocket.Conn) {
	...
	if factory.PongTimeout > 0 {
		// pongTimeout is the effective read-deadline extension window.
