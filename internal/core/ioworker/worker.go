...
func NewWorker(ch *Channels, log *logger.Logger) *Worker {
	return &Worker{
		ch:  ch,
		log: log,
	}
...
func (w *Worker) handleRefresh(ctx context.Context, req RefreshRequest) {
	w.log.Debug("refresh started", "account", req.AccountID)

	result := BaselineResult{AccountID: req.AccountID}
...
func (w *Worker) handleAction(ctx context.Context, req ActionRequest) {
