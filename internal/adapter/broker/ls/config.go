...
const (
	...
	// WSURLReal is the real-trading WebSocket endpoint.
	WSURLReal = "wss://openapi.ls-sec.co.kr:9443/websocket"

	// WSURLPaper is the paper-trading WebSocket endpoint.
	WSURLPaper = "wss://openapi.ls-sec.co.kr:29443/websocket"
)
...
type Config struct {
	...
	AccountNo string
	// IsPaper selects the paper-trading websocket endpoint when true.
	IsPaper bool
}

// ResolveWSURL returns the configured LS realtime endpoint for the account mode.
func ResolveWSURL(isPaper bool) string {
	if isPaper {
		return WSURLPaper
	}
	return WSURLReal
